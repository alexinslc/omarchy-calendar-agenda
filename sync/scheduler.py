...
def install_timer(
    ...
    *,
    ...
    try:
        ...
        raise SchedulerError(f"cannot install synchronization timer: {error}") from error
    ...
    try:
        ...
        if failure is None:
            failure = SchedulerError(
                f"cannot remove synchronization timer files: {error}"
            )
    try:
        _run(["systemctl", "--user", "daemon-reload"], runner)
    except SchedulerError as error:
